...
\begin{equation}
  \label{eq:dynsgd_ps}
  \tilde{\theta}_{t+1} = \tilde{\theta}_t - \frac{1}{\tau_i} \odot \eta_t \odot \nabla_\theta \mathcal{L}(\theta^k_t;\mathbf{x}^k_t;\mathbf{y}^k_t)
\end{equation}
...
\begin{algorithm}[H]
  \caption{Zealous worker procedure of \textsc{downpour}, inspired by~\cite{louppe2010zealous}.}
  \label{algo:dynsgd_paramter_server}
  \begin{algorithmic}[1]
    \Procedure{DynSGDParameterServer}{$k$}
    \State $\theta^k_0 \gets \tilde{\theta} \gets \Call{Pull}$
    \State $t \gets 0$
    \While{$\textbf{not}$ converged}
    \State $i \gets 0$

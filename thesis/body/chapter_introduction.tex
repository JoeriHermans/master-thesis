...
However, what happens when the computing resources on your local machine are constrained? That is, due to the limited amount of available CPU cores (or even GPU's) your parallization of the mini-batch computation doesn't scale very well on your local machine.\\

In \emph{Asynchronous Data Parallelism} on the other hand, there is no global locking mechanism. This implies that whenever a worker wants to submit a new result, it is able to modify the central model.\\
